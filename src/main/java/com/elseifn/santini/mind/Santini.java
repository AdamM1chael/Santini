package com.elseifn.santini.mind;
...
import com.elseifn.santini.model.data.MindData;
...
public class Santini {
  ...
  /**
   * Returns the current state of Santini. This relates to whether the bot is waiting for a buy back
   * or not
   *
   * @return String for the UI stating Santini's status
   */
  ...
  /**
   * Sets the credentials that are needed for tweeting alerts when Santini decides to sell and buy
   * back.
   *
  ...
  public void predictAndTrade() {
    ...
    // Gather data calculate, and update Santini's target price and buy back
  ...
  private void executeMarketBuyBack() {
    ...
    Double btcToBuyFloored = CalcUtils.floorTo(btcToBuy, 4);
  ...
  private void reportDevMode() {
    logger.trace("Santini is currently in development mode! Not performing trades or tweets");
  }
}
